...
if (!gl || !ext || !extFloat || !extDepth || !extDeriv) {
  ...
  float blurAO(vec2 screenSpaceOrigin) {
    ...
    float originDepth = texture2D(depthBuf, screenSpaceOrigin).x;
    ...
    for (int x = -4; x <= 4; x++) {
      for (int y = -4; y <= 4; y++) {
        if (x != 0 || y != 0) {
          ...
          float sampleDepth = texture2D(depthBuf, samplePosition).x;
          int kx = 4 - (x < 0 ? -x : x);
          int ky = 4 - (y < 0 ? -y : y);
  ...
  void main() {
    ...
    vec3 depthAtOrigin = texture2D(depthBuf, screenSpaceOrigin).xyz;
    ...
    if (showAll) {
      ...
      float d = (depthAtOrigin.x - 0.9)/0.1;
      gl_FragColor = vec4(d, d, d, 1.0);
    } else if (showPosition) {
      gl_FragColor = vec4((10.0 + worldSpaceOrigin)/20.0, 1.0);
    } else if (showNormal) {
      gl_FragColor = vec4(normalAtOrigin, 1.0);
    } else if (showAO) {
      gl_FragColor = vec4(aoAtOrigin, 1.0);
    } else if (showBlurredAO) {
      gl_FragColor = vec4(occlusion, occlusion, occlusion, 1.0);
    } else if (showDiffuse) {
      gl_FragColor = vec4(colorAtOrigin, 1.0);
    }
  }
  `;
  ...
  const draw = () => {
    ...
    gl.bindFramebuffer(gl.FRAMEBUFFER, geometryFB);
    gl.clearColor(0.97, 0.95, 0.73, 1.0);
    gl.clearDepth(1.0);
    ...
    gl.uniform1i(infoFinal.showAll, useAll.checked);
